...
export default function Hero() {
  ...
  useGSAP(
    () => {
      ...
      // Las cards tienen `transition-transform`/`transition` (Tailwind) para el hover;
      // eso compite con los transforms que GSAP escribe cuadro a cuadro. Se desactiva
      // mientras corre la entrada y se restaura al final para no romper el hover.
      const transitioned =
        ".hero-card-headline, .hero-card-photo, .hero-card-role, .hero-card-connect, .hero-social"
      gsap.set(transitioned, { transition: "none" })

      const tl = gsap.timeline({
        defaults: { ease: "power3.out", duration: 0.8 },
        onComplete: () => gsap.set(transitioned, { clearProps: "transition" }),
      })

      tl.from(".hero-card-headline", { y: 50, opacity: 0 })
        .from(
          [".hero-eyebrow", ".hero-headline"],
          { y: 14, opacity: 0, duration: 0.6, stagger: 0.12 },
          "-=0.5",
        )
        .from(".hero-card-photo", { scale: 0.9, opacity: 0, duration: 0.9 }, "-=0.7")
        .from(".hero-available-badge", { y: -10, opacity: 0, duration: 0.5 }, "-=0.4")
        .from(".hero-card-role", { x: -40, opacity: 0 }, "-=0.6")
        .from(".hero-card-connect", { x: 40, opacity: 0 }, "-=0.6")
        .from(
          ".hero-connector",
          { scale: 0, rotate: -45, duration: 0.5, ease: "back.out(2)" },
          "-=0.3",
        )
        .from(
          ".hero-social",
          { y: 14, opacity: 0, scale: 0.7, duration: 0.5, stagger: 0.1, ease: "back.out(2)" },
          "-=0.2",
        )
    },
    { scope: containerRef },
  ...
  return (
    <section
      ref={containerRef}
      className="relative isolate overflow-hidden bg-gradient-to-b from-background to-primary/[0.05] px-4 pb-16 pt-28 sm:px-6 lg:flex lg:min-h-screen lg:items-center lg:px-10"
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl">
        <div className="relative flex flex-col gap-3 sm:gap-4 lg:grid lg:grid-cols-6 lg:gap-4">
          ...
          <span
            aria-hidden
            className="hero-connector blob pointer-events-none absolute left-[63.5%] top-[12%] z-20 hidden h-[70px] w-[70px] rotate-12 bg-primary lg:block"
          />


          {/* A · Titular */}
          <div className="hero-card-headline group relative flex flex-col justify-between overflow-hidden rounded-[2rem] border border-border bg-card/95 p-7 shadow-[0_28px_70px_-34px_rgba(242,68,121,0.35)] backdrop-blur-sm transition-transform duration-300 ease-out hover:-translate-y-1 sm:p-9 lg:col-span-4 lg:col-start-1 lg:row-span-2 lg:row-start-1 dark:border-white/10">
            <div>
              <span className="hero-eyebrow inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.22em] text-primary">
                {t.eyebrow}
              </span>
              <h1 className="hero-headline mt-4 text-3xl font-bold leading-[1.1] text-balance sm:text-4xl xl:text-5xl">
                {t.headlinePre}
          ...
          {/* B · Foto */}
          <div className="hero-card-photo group relative min-h-[300px] overflow-hidden rounded-[2rem] border border-border shadow-[0_28px_70px_-34px_rgba(242,68,121,0.4)] transition-transform duration-300 ease-out hover:-translate-y-1 lg:col-span-2 lg:col-start-5 lg:row-span-3 lg:row-start-1 lg:min-h-0 lg:rotate-[0.6deg] lg:hover:rotate-0 dark:border-white/10">
            <img
              src="/fotosyra.jpg"
              alt="Syra Moran"
            ...
            <span className="hero-available-badge absolute left-4 top-4 rounded-full bg-background/85 px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider backdrop-blur">
              {t.available}
            </span>
          ...
          {/* C · Nombre / rol */}
          <div className="hero-card-role group relative flex flex-col justify-between gap-6 overflow-hidden rounded-[2rem] bg-[#141414] p-7 text-white shadow-[0_20px_50px_-24px_rgba(0,0,0,0.5)] ring-1 ring-white/10 transition-transform duration-300 ease-out hover:-translate-y-1.5 lg:col-span-2 lg:col-start-1 lg:row-start-3 lg:min-h-[200px] lg:-rotate-1 lg:hover:rotate-0">
            <div>
          ...
          {/* D · Conectá */}
          <div className="hero-card-connect group relative flex flex-col justify-between gap-4 overflow-hidden rounded-[2rem] bg-primary p-7 text-white shadow-[0_20px_50px_-24px_rgba(242,68,121,0.6)] transition-transform duration-300 ease-out hover:-translate-y-1.5 lg:col-span-2 lg:col-start-3 lg:row-start-3 lg:min-h-[200px] lg:rotate-[1.4deg] lg:hover:rotate-0">
            <div>
            ...
            <div className="flex items-center gap-2.5">
              <a
                ...
                aria-label="LinkedIn"
                className="hero-social grid h-11 w-11 place-items-center rounded-full border border-white/30 bg-white/15 transition duration-300 ease-out hover:-translate-y-0.5 hover:bg-white/25"
              >
                <Linkedin size={17} />
              </a>
              <a
